...
export function LoginForm() {
  ...
  const handlePasswordReset = async () => {
    try {
      forgotPasswordSchema.parse({ resetEmail }); // Validate email before sending
    ...
    <>
      <Card className="w-full max-w-md shadow-xl">
        ...
        <CardFooter className="flex flex-col items-center space-y-4 pt-4">
            <Button variant="link" size="sm" className="text-sm text-primary p-0 h-auto" onClick={() => setShowForgotPasswordDialog(true)}>
                Forgot Password?
            </Button>
